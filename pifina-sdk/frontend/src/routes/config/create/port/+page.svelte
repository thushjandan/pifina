...
<header class="bg-white shadow">
    <div class="mx-auto max-w-7xl px-4 py-6 sm:px-6 lg:px-8">
      <h1 class="text-3xl font-bold tracking-tight text-gray-900">Add new port</h1>
    </div>
</header>
<main>
    <div class="mx-auto max-w-7xl py-6 sm:px-6 lg:px-8">
        <div class="bg-white rounded-lg px-8 py-8 shadow-lg">
            ...
            {/if}
            <p class="mb-2 text-lg text-gray-500 md:text-xl dark:text-gray-400">From which ports would you like to see the traffic manager metrics. Enter the name of the port.</p>
            <form on:submit|preventDefault={handleSubmit}>
                    <div class="grid gap-6 mb-6 md:grid-cols-2">
                        <div>
                            <label for="first_name" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Port name</label>
                            <div class="flex">
                                <input type="text" bind:value={newEntry.name} class="rounded-lg bg-gray-50 border text-gray-900 focus:ring-indigo-500 focus:border-indigo-500 block flex-1 min-w-0 w-full text-sm border-gray-300 p-2.5  dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-indigo-500 dark:focus:border-indigo-500" placeholder="2/0" required>
                            </div>
                        </div>
                    </div>
                ...
                <button on:click={() => showModal = !showModal} type="button" class="py-2.5 px-5 mr-2 text-sm font-medium text-gray-900 bg-white rounded-lg border border-gray-200 hover:bg-gray-100 hover:text-indigo-700">Show all available ports</button>
            ...
            {#await availableRegPromise then data }
                ...
                {#each data as entry}
                    <li><a href={undefined} on:click|preventDefault={() => newEntry.name = entry.name}>{entry.name}</a></li>
                {/each}
                </ul>
            {/await}     
            </div>

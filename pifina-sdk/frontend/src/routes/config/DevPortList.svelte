    ...
    function deleteProbe() {
        if (typeof targetPortToDelete !== 'undefined') {
            ...
            fetch(`/api/v1/ports?endpoint=${localEndpointAddress}`, {
                ...
                body: JSON.stringify(targetPortToDelete)
            }).then(data => {
                loading = false;
                showModal = false;
                closeModal();
...
{#await devPortPromise }
    wait
{:then data }
<div class="relative overflow-x-auto mt-8">
    <h2 class="mb-4 text-3xl font-bold dark:text-white">TM stats are retrieved for following ports</h2>
    <button type="button" on:click={() => goto(`/config/create/port`)} class="mb-4 text-white bg-indigo-500 hover:bg-indigo-800 font-medium rounded-lg text-sm p-2.5 text-center inline-flex items-center">
        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5 mr-1">
        <path stroke-linecap="round" stroke-linejoin="round" d="M12 4.5v15m7.5-7.5h-15" />
        </svg>
        Add new port
    <span class="sr-only">Add new port</span>
    </button>
